...
class FlowProjectionLayer(Function):
    ...
    def forward(ctx, input1, requires_grad):
        ...
        if input1.is_cuda:
            ...
            err = my_lib.FlowProjectionLayer_gpu_forward(
                input1, count, output, fillhole
            )
        else:
            output = torch.cuda.FloatTensor(input1.data.size())
            err = my_lib.FlowProjectionLayer_cpu_forward(
                input1, count, output, fillhole
            )
        if err != 0:
            print(err)
    ...
    def backward(ctx, gradoutput):
        ...
        if input1.is_cuda:
            ...
            err = my_lib.FlowProjectionLayer_gpu_backward(
            ...
            err = my_lib.FlowProjectionLayer_cpu_backward(
                input1, count, gradoutput, gradinput1
            )
            # print(err)
            if err != 0:
