...
class S2DF(nn.Module):
    def __init__(self, block, num_blocks, dense=True, dilation=True):
        ...
        self.block2 = (
            block(self.inplanes, 64, dilation=4 if dilation else 1)
            if num_blocks >= 2
            else None
        )
        self.block3 = (
            block(self.inplanes, 64, dilation=8 if dilation else 1)
            if num_blocks >= 3
            else None
        )
        self.block4 = (
            block(self.inplanes, 64, dilation=16 if dilation else 1)
            if num_blocks >= 4
            else None
        )
...
class S2DFsim(nn.Module):
    def __init__(self, block, num_blocks, dense=True, dilation=True):
        ...
        #     elif isinstance(m, nn.BatchNorm2d):
        #         m.weight.data_loader.fill_(1)
        #         m.bias.data_loader.zero_()
