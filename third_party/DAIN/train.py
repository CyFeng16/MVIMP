...
from third_party.DAIN import datasets
from third_party.DAIN import balancedsampler
...
def train():
    ...
    if type(args.datasetName) == list:
        ...
        for ii, jj in zip(args.datasetName, args.datasetPath):
            tr_s, te_s = datasets.__dict__[ii](
                jj, split=args.dataset_split, single=args.single_output, task=args.task
            )
    ...
    if hasattr(model, "flownets"):
        print(
            "Num. of flow model parameters is :"
    ...
    for t in range(args.numEpoch):
        ...
        for i, (X0_half, X1_half, y_half) in enumerate(train_loader):
            ...
            total_loss = sum(
                x * y if x > 0 else 0 for x, y in zip(args.alpha, pixel_loss)
            )
        ...
        for i, (X0, X1, y) in enumerate(val_loader):
            ...
            with torch.no_grad():
                ...
                per_sample_pix_error = torch.mean(
                    torch.mean(torch.mean(diffs[args.save_which] ** 2, dim=1), dim=1),
                    dim=1,
                )
        ...
        print(
            ...
            + "\tTV Loss: "
            + str([round(float(val_total_tv_loss.avg), 4)])
            + "\tPWS Loss: "
            + str([round(float(val_total_pws_loss.avg), 4)])
            + "\tSym Loss: "
            + str([round(float(val_total_sym_loss.avg), 4)])
        )
        ...
        auxiliary_data.append(
            [
                ...
                float(ikk["lr"]),
                training_losses.avg,
                val_total_losses.avg,
                val_total_pixel_loss.avg,
                val_total_tv_loss.avg,
                val_total_pws_loss.avg,
                val_total_sym_loss.avg,
            ]
        )
...
if __name__ == "__main__":
    sys.setrecursionlimit(
        100000
    )  # 0xC00000FD exception for the recursive detach of gradients.
