...
def parse_args():
    ...
    parser.add_argument(
        "--output", type=str, default="video/output/" + "Paprika", help="output path"
    )
    parser.add_argument(
        "--output_format",
        type=str,
...
def getfileloc(
    ...
    filetypes=(("video files", ".mkv .avi .mp4"), ("all files", "*.*")),
):
    root = tk.Tk()
    if method == "open":
        ...
        fileloc = filedialog.asksaveasfilename(
            parent=root,
...
def cvt2anime_video(
    video, output, checkpoint_dir, output_format="MP4V", img_size=(256, 256)
):
    """
    ...
    with tf.compat.v1.Session(config=tfconfig) as sess:
        ...
        out = cv2.VideoWriter(
            os.path.join(output, vid_name), codec, fps, (width, height)
        )
...
if __name__ == "__main__":
    ...
    if not arg.output:
        ...
        arg.output = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), arg.output
        )
    check_folder(arg.output)
    info = cvt2anime_video(
        arg.video, arg.output, arg.checkpoint_dir, output_format=arg.output_format
    )
