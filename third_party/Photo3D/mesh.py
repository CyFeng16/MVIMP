...
def create_mesh(depth, image, int_mtx, config):
    ...
    LDI.graph["cam_param_pix"], LDI.graph["cam_param_pix_inv"] = (
        int_mtx_pix,
        np.linalg.inv(int_mtx_pix),
    )
    disp = 1.0 / (-depth)
    ...
    for nx, ny in zip(nxs, nys):
        ...
        for fne in four_nes:
            if mesh.has_node((fne[0], fne[1], end_maps[fne[0], fne[1]])):
                node_a, node_b = (
                    (fne[0], fne[1], end_maps[fne[0], fne[1]]),
...
def context_and_holes(
    ...
    for edge_id, edge_cc in enumerate(edge_ccs):
        ...
        if inpaint_iter == 0:
            ...
            near_depth_map, raw_near_depth_map = (
...
def DL_inpaint_edge(
    ...
    for (
        edge_id,
        (context_cc, mask_cc, erode_context_cc, extend_context_cc, edge_cc),
    ) in enumerate(
