...
def extrapolate(
    ...
    if "up" in direc.lower() and "-" not in direc.lower():
        ...
        context_anchor = [
            h_off,
            h_off + config["context_thickness"],
            w_off,
            w_off + noext_W,
        ]
        ...
        all_anchor = [
            h_off + noext_H - config["context_thickness"],
            2 * h_off + noext_H,
            w_off,
            w_off + noext_W,
        ]
        ...
        all_anchor = [
            h_off,
            h_off + noext_H,
            w_off + noext_W - config["context_thickness"],
            2 * w_off + noext_W,
        ]
        global_shift = [all_anchor[0], all_anchor[2]]
    ...
    mask = (
        global_mask[
            valid_anchor[0] : valid_anchor[1], valid_anchor[2] : valid_anchor[3]
        ]
        * 1
    )
    context = 1 - mask
    ...
    global_context[
        all_anchor[0] : all_anchor[1], all_anchor[2] : all_anchor[3]
    ] = context
    # context = global_context[valid_anchor[0]:valid_anchor[1], valid_anchor[2]:valid_anchor[3]] * 1

    valid_area = mask + context
    input_rgb = (
        image[valid_anchor[0] : valid_anchor[1], valid_anchor[2] : valid_anchor[3]]
        / 255.0
        * context[..., None]
    )
    input_depth = (
        depth[valid_anchor[0] : valid_anchor[1], valid_anchor[2] : valid_anchor[3]]
        * context
    )
    ...
    input_disp = 1.0 / np.abs(input_depth)
    input_disp[mask > 0] = 0
    ...
    # return
    input_edge_map = (
        edge_map[all_anchor[0] : all_anchor[1], all_anchor[2] : all_anchor[3]] * context
    )
    ...
    valid_edge_ids = (
        valid_edge_ids[1:]
        if (len(valid_edge_ids) > 0 and valid_edge_ids[0] == -1)
        else valid_edge_ids
    )
    edge = reduce(
    ...
    t_depth_zero_mean_depth = torch.FloatTensor(input_zero_mean_depth).to(device)[
        None, None, ...
    ]

    depth_edge_output = depth_edge_model.forward_3P(
        t_mask, t_context, t_rgb, t_disp, t_edge, unit_length=128, cuda=device
    )
    t_output_edge = (
    ...
    for hx, hy in zip(hxs, hys):
        ...
        eight_nes = [
            ...
            if 0 <= ne[0] < output_raw_edge.shape[0]
            and 0 <= ne[1] < output_raw_edge.shape[1]
            and 0 < output_raw_edge[ne[0], ne[1]]
        ]
        for ne in eight_nes:
    ...
    for end_pt, cc in zip(end_pts, ccs):
        ...
        if len(end_pt) == 1:
            ...
            edge_id = global_mesh.nodes[
                (ends[0][0] + all_anchor[0], ends[0][1] + all_anchor[2], -ends[0][2])
            ]["edge_id"]
            pnodes = sorted(
                pnodes,
            ...
            if (
                global_mesh.nodes[
                    (
                        ends[0][0] + all_anchor[0],
                        ends[0][1] + all_anchor[2],
                        -ends[0][2],
                    )
                ].get("far")
                is None
                ...
                fnodes = [
                    (xx[0] - all_anchor[0], xx[1] - all_anchor[2], xx[2])
                    for xx in fnodes
                ]
                dmask = mask + 0
                did = 0
                ...
                for step in range(0, len(npath) - 1):
                    ...
                    if (
                        np.sum([fpath_map[nlne[0], nlne[1]] for nlne in new_loc_nes])
                        != -4
                    ):
                        break
                    ...
                    if (
                        np.sum(
                            [
                                int(npath_map[nlne[0], nlne[1]] == edge_id)
                                for nlne in new_loc_nes_eight
                            ]
                        )
                        == 0
                    ):
    ...
    depth_output = (
        np.exp(depth_output + input_mean_depth) * mask
    )  # + input_depth * context
    ...
    rgb_output = rgb_feat_model.forward_3P(
        t_mask, t_context, t_rgb, t_update_edge, unit_length=128, cuda=device
    )
    ...
    rgb_output = (
        (
            rgb_output.squeeze().data.cpu().permute(1, 2, 0).numpy() * mask[..., None]
            + input_rgb
        )
        * 255
    ).astype(np.uint8)
    image[all_anchor[0] : all_anchor[1], all_anchor[2] : all_anchor[3]][
        mask > 0
    ] = rgb_output[
        mask > 0
    ]  # np.array([255,0,0]) # rgb_output[mask > 0]
    depth[all_anchor[0] : all_anchor[1], all_anchor[2] : all_anchor[3]][
        mask > 0
    ] = depth_output[mask > 0]
            ...
            if fpath_map[nex, ney] == n_id:
                na, nb = (
                    ...
                    (
                        nex + all_anchor[0],
                        ney + all_anchor[2],
                        info_on_pix[(nex + all_anchor[0], ney + all_anchor[2])][0][
                            "depth"
                        ],
                    ),
                )
    ...
    for fpath_id in fpath_ids:
        fpath_real_id = np.unique(
            ((global_fpath_map == fpath_id).astype(np.int) * (other_edge_with_id + 1))
            - 1
        )
        fpath_real_id = (
            fpath_real_id[1:]
            if fpath_real_id.shape[0] > 0 and fpath_real_id[0] == -1
            else []
        )
    ...
    for nx, ny in zip(nxs, nys):
        ...
        for nex, ney in four_nes:
            ...
            if global_mesh.has_node(ne_node) is False:
                continue
            if (
                npath_map[nex, ney] == n_id
                or global_mesh.nodes[ne_node].get("edge_id") == n_id
            ):
                if global_mesh.has_edge(self_node, ne_node) is True:
                    global_mesh.remove_edge(self_node, ne_node)
...
def size_operation(size_a, size_b, operation):
    ...
    ), "Operation must be '+' (union) or '-' (exclude)"
    osize = {}
    if operation == "+":
...
def fill_dummy_bord(mesh, info_on_pix, image, depth, config):
    ...
    for x, y in zip(xs, ys):
        ...
        four_nes = [
            ...
            and info_on_pix.get((xx, yy)) is not None
        ]
        for ne in four_nes:
            # if (ne[0] - x) + (ne[1] - y) == 1 and info_on_pix.get((ne[0], ne[1])) is not None:
            mesh.add_edge(
                cur_node, (ne[0], ne[1], info_on_pix[(ne[0], ne[1])][0]["depth"])
            )
...
def enlarge_border(mesh, info_on_pix, depth, image, config):
    mesh.graph["hoffset"], mesh.graph["woffset"] = (
        config["extrapolation_thickness"],
        config["extrapolation_thickness"],
    )
    (
        mesh.graph["bord_up"],
        mesh.graph["bord_left"],
        mesh.graph["bord_down"],
...
def refresh_bord_depth(mesh, info_on_pix, image, depth):
    ...
    corner_nodes = [
        (mesh.graph["bord_up"], mesh.graph["bord_left"]),
    ...
    bord_nodes = []
    bord_nodes += [
        (mesh.graph["bord_up"], xx)
        for xx in range(mesh.graph["bord_left"] + 1, mesh.graph["bord_right"] - 1)
    ]
    bord_nodes += [
        (mesh.graph["bord_down"] - 1, xx)
        for xx in range(mesh.graph["bord_left"] + 1, mesh.graph["bord_right"] - 1)
    ]
    bord_nodes += [
        (xx, mesh.graph["bord_left"])
        for xx in range(mesh.graph["bord_up"] + 1, mesh.graph["bord_down"] - 1)
    ]
    bord_nodes += [
        (xx, mesh.graph["bord_right"] - 1)
        for xx in range(mesh.graph["bord_up"] + 1, mesh.graph["bord_down"] - 1)
    ]
    for xy in bord_nodes:
        ...
        if tgt_loc is not None:
            ...
            tgt_nes_loc = [(xx[0], xx[1]) for xx in mesh.neighbors(src_node)]
            tgt_nes_loc = [
                (xx[0] - tgt_loc[0] + xy[0], xx[1] - tgt_loc[1] + xy[1])
                for xx in tgt_nes_loc
            ...
            if info_on_pix.get(xy) is not None and len(info_on_pix.get(xy)) > 0:
                ...
                mesh.add_edges_from(
                    [
                        ((zz[0], zz[1], info_on_pix[zz][0]["depth"]), old_node)
                        for zz in tgt_nes_loc
                    ]
                )
                ...
                mesh.add_edges_from(
                    [
                        ((zz[0], zz[1], info_on_pix[zz][0]["depth"]), new_node)
                        for zz in tgt_nes_loc
                    ]
                )
            ...
            if mesh.nodes[src_node].get("far") is not None:
                redundant_nodes = [
                    ne for ne in mesh.nodes[src_node]["far"] if (ne[0], ne[1]) == xy
                ]
                [mesh.nodes[src_node]["far"].remove(aa) for aa in redundant_nodes]
            if mesh.nodes[src_node].get("near") is not None:
                redundant_nodes = [
                    ne for ne in mesh.nodes[src_node]["near"] if (ne[0], ne[1]) == xy
                ]
    ...
    for xy in corner_nodes:
        ...
        four_nes = [
            xx
            for xx in [(hx + 1, hy), (hx - 1, hy), (hx, hy + 1), (hx, hy - 1)]
            if mesh.graph["bord_up"] <= xx[0] < mesh.graph["bord_down"]
            and mesh.graph["bord_left"] <= xx[1] < mesh.graph["bord_right"]
        ]
        ne_nodes = []
        ne_depths = []
    ...
    for xy in bord_nodes:
        ...
        four_nes = [
            ne
            for ne in four_nes
            if mesh.graph["bord_up"] <= ne[0] < mesh.graph["bord_down"]
            and mesh.graph["bord_left"] <= ne[1] < mesh.graph["bord_right"]
        ]
        four_nes = [
            (ne[0], ne[1], info_on_pix[(ne[0], ne[1])][0]["depth"]) for ne in four_nes
        ]
...
def incomplete_node(mesh, edge_maps, info_on_pix):
    ...
    for node in mesh.nodes:
        if mesh.nodes[node].get("synthesis") is not True:
            ...
            nes = [
                xx
                for xx in mesh.neighbors(node)
                if mesh.nodes[xx].get("synthesis") is not True
            ]
            if (
                len(nes) < 3
                and 0 < node[0] < mesh.graph["H"] - 1
                and 0 < node[1] < mesh.graph["W"] - 1
            ):
                if len(nes) <= 1:
...
def depth_inpainting(
    ...
    resize_mask = open_small_mask(
        tensor_depth_dict["mask"], tensor_depth_dict["context"], 3, 41
    )
    with torch.no_grad():
        device = (
            config["gpu_ids"]
    ...
    tensor_depth_dict["output"] = (
        torch.exp(depth_output + depth_dict["mean_depth"]) * tensor_depth_dict["mask"]
        + tensor_depth_dict["depth"]
    )
    patch_depth_dict["output"] = (
        tensor_depth_dict["output"].data.cpu().numpy().squeeze()
    )
    ...
    depth_output = (
        ...
        + depth_dict["depth"] * depth_dict["context"]
    )
    depth_output = smooth_cntsyn_gap(
        depth_dict["output"].copy() * depth_dict["mask"]
        + depth_dict["depth"] * depth_dict["context"],
        depth_dict["mask"],
        depth_dict["context"],
    ...
    if spdb is True:
        ...
        pdb.set_trace()
    depth_dict["output"] = (
        depth_output * depth_dict["mask"] + depth_dict["depth"] * depth_dict["context"]
    )
...
def update_info(mapping_dict, info_on_pix, *meshes):
    ...
    for mesh in meshes:
        rt_meshes.append(
            relabel_node(
                mesh, mesh.nodes, [*mapping_dict.keys()][0], [*mapping_dict.values()][0]
            )
        )
    ...
    for node_xy in ne_nodes:
        ...
        if mark[node[0], node[1]] != 3:
            ...
            info_on_pix, edge_mesh, mesh = update_info(
                mapping_dict, info_on_pix, edge_mesh, mesh
            )

            edge_mesh, mesh, mark, info_on_pix = recursive_add_edge(
                edge_mesh, mesh, info_on_pix, re_node, mark
            )
...
def resize_for_edge(tensor_dict, largest_size):
    ...
    if frac < 1:
        resize_mark = torch.nn.functional.interpolate(
            torch.cat((resize_dict["mask"], resize_dict["context"]), dim=1),
            scale_factor=frac,
            mode="bilinear",
        )
        ...
        resize_dict["edge"] = torch.nn.functional.interpolate(
            resize_dict["edge"], scale_factor=frac, mode="bilinear"
        )
...
def repaint_info(mesh, cc, x_anchor, y_anchor, source_type):
    ...
    for node in cc:
        if source_type == "rgb":
            feat[:, node[0] - x_anchor[0], node[1] - y_anchor[0]] = (
                np.array(mesh.nodes[node]["color"]) / 255.0
            )
        elif source_type == "d":
...
def get_context_from_nodes(mesh, cc, H, W, source_type=""):
    ...
    for node in cc:
        if "rgb" in source_type or "color" in source_type:
            feat[node[0], node[1]] = np.array(mesh.nodes[node]["color"]) / 255.0
...
def get_rgb_from_nodes(context_cc, erode_context_cc, mask_cc, H, W, mesh):
    ...
    for node in erode_context_cc:
        ...
        mask[node[0], node[1]] = 1
    rt_dict = {"rgb": rgb, "context": context, "mask": mask, "erode": erode_context}
