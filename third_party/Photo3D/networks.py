...
class PartialConv(nn.Module):
    ...
    def forward(self, input, mask):
        ...
        output_pre = (
            (output - output_bias) * self.slide_winsize
        ) / mask_sum + output_bias
...
class Inpaint_Depth_Net(nn.Module):
    def __init__(self, layer_size=7, upsampling_mode="nearest"):
        ...
        self.enc_1 = PCBActiv(
            in_channels, 64, bn=False, sample="down-7", conv_bias=True
        )
        ...
        self.dec_1 = PCBActiv(
            64 + in_channels, out_channels, bn=False, activ=None, conv_bias=True
        )
    ...
    def forward_3P(self, mask, context, depth, edge, unit_length=128, cuda=None):
        with torch.no_grad():
            ...
            depth_output = depth_output[
                ..., anchor_h : anchor_h + h, anchor_w : anchor_w + w
            ]
    ...
    def forward(self, input_feat, refine_border=False, sample=False, PCONV=True):
        ...
        if refine_border is True:
            ...
            input_mask, anchor = self.add_border(
                input_mask, mask_flag=True, PCONV=PCONV
            )
...
class Inpaint_Edge_Net(BaseNetwork):
    def __init__(self, residual_blocks=8, init_weights=True):
        ...
        self.decoder_2 = nn.Sequential(
            nn.ReflectionPad2d(3),
            nn.Conv2d(
                in_channels=64 + 64, out_channels=out_channels, kernel_size=7, padding=0
            ),
        )
...
class Inpaint_Color_Net(nn.Module):
    def __init__(
        ...
        self.dec_1A = PCBActiv(
            64 + in_channels, 3, bn=False, activ=None, conv_bias=True
        )
        """
