...
SITE_TO_KNOWN_GOOD_FILE = {"github": "known_good.json", "gitlab": "known_good_khr.json"}

# Maps a site name to its hostname.
SITE_TO_HOST = {"github": "https://github.com/", "gitlab": "git@gitlab.khronos.org:"}
...
def command_output(cmd, directory, fail_ok=False):
    ...
    p = subprocess.Popen(cmd, cwd=directory, stdout=subprocess.PIPE)
...
class GoodCommit(object):
    ...
    def HasCommit(self):
        ...
        return 0 == subprocess.call(
            ["git", "rev-parse", "--verify", "--quiet", self.commit + "^{commit}"],
            cwd=self.subdir,
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Get Glslang source dependencies at a known-good commit"
    )
    parser.add_argument(
        "--dir",
        dest="dir",
